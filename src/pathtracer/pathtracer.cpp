...
Vector3D PathTracer::at_least_one_bounce_radiance(const Ray &r,
  ...
  if (r.depth > 0 && coin_flip(cpdf)) {
      ...
      if (bvh->intersect(in, &temp)) {
          in.min_t = EPS_F;
          in.max_t = INF_D;
          Vector3D samp = at_least_one_bounce_radiance(in, temp);
          if (isect.bsdf->is_delta())
              samp += zero_bounce_radiance(in, temp);
          L_out += samp * reflectance / pdf / cpdf * lambersion;
      }
  }
  return L_out;
}
