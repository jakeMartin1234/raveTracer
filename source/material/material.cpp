...
glm::dvec3 Material::specularTransmissionCustom(const glm::dvec3& wi, const glm::dvec3& wo, double n1,
                                          ...
                                          glm::dvec3& transmittanceCustom) const
{
    ...
    glm::dvec3 btdf = !inside ? transmittanceCustom : glm::dvec3(1.0);
    if (rough_specular)
    {
        btdf *= GGX::transmission(wi, wo, n1, iorCustom, a, PDF);
        if (flux) btdf *= pow2(n2 / iorCustom);
    }
    else
    {
        PDF = 1.0;
        btdf *= transmittanceCustom / std::abs(wi.z);
        if (!flux) btdf *= pow2(n1 / iorCustom);
    }
    return btdf;
}
...
void from_json(const nlohmann::json &j, Material &m)
{
    ...
    getToOptional(j, "diffractivity", m.diffractivity);
